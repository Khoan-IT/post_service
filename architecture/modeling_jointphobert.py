...
class JointPhoBERT(RobertaPreTrainedModel):
    def __init__(self, config, args, intent_label_lst, slot_label_lst):
        ...
        self.list_cnn = nn.ModuleList([nn.Conv1d(config.hidden_size, 256, i, device='cpu', padding='same') for i in range(5,8)]) 
        self.config = config
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, intent_label_ids, slot_labels_ids):
        ...
        input_cnn = sequence_output.permute(0, 2, 1)
        result = []
        for layer in self.list_cnn:
          n_gram_result = layer(input_cnn)
          result.append(n_gram_result)
        out_cnn = torch.cat(result, 1)
        out_cnn_original = out_cnn.permute(0, 2, 1)
        sequence_output = out_cnn_original
